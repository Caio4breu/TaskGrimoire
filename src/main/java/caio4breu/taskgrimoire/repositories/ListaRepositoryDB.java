...
public class ListaRepositoryDB implements ListaRepository {
    ...
    public ListaDeAtividades buscarPorNome(String nome){
        String sql = "select id, nome, tipo from lista_atividades where nome = ?";
        try (Connection conn = ConexaoDB.conectar();
            PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                ListaDeAtividades lista = new ListaDeAtividades(rs.getString("nome"), tipo);
    ...
    public List<ListaDeAtividades> buscarTodos() {
        ...
        String sql = "select id, nome, tipo from lista_atividades";
        try (Connection conn = ConexaoDB.conectar();
            PreparedStatement stmt = conn.prepareStatement(sql)) {
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
